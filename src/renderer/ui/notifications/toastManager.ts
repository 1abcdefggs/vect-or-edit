...
export function showToast(message: string, type: ToastType = 'info', action?: ToastAction | ToastAction[]): void {
  ...
  const actionsList: ToastAction[] = Array.isArray(action) ? action : (action && action.label ? [action] : []);
  const actionHtml = actionsList.map((act, idx) => {
    const isSecondary = act.secondary;
